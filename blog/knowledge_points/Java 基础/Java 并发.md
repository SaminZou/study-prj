...
上述 demo 代码，线程池每次会同时执行 5 个任务，这 5 个任务执行完之后，剩余的 5 个任务才会被执行。
...
主要用来在给定的延迟后运行任务，或者定期执行任务，这个在实际项目中基本不会被用到，因为有其他方案选择比如 Quartz
...
2. 对于资源竞争严重（线程冲突严重）的情况，CAS 自旋的概率会比较大，从而浪费更多的CPU资源，效率低于 synchronized
