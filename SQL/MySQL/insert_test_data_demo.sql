...
BEGIN
    DECLARE
first_names VARCHAR(255) DEFAULT 'John,Mary,Michael,Sophia,James,Emma,William,Olivia';
    DECLARE
last_names VARCHAR(255) DEFAULT 'Smith,Johnson,Williams,Jones,Brown,Taylor,Miller,Wilson';
    DECLARE
first_name VARCHAR(255);
    DECLARE
last_name VARCHAR(255);
    SET
first_name = SUBSTRING_INDEX(SUBSTRING_INDEX(first_names, ',', FLOOR(1 + RAND() * (LENGTH(first_names) - 1))), ',', -1);
    SET
last_name = SUBSTRING_INDEX(SUBSTRING_INDEX(last_names, ',', FLOOR(1 + RAND() * (LENGTH(last_names) - 1))), ',', -1);
...
BEGIN
    DECLARE
prefix VARCHAR(1) DEFAULT '1';
    DECLARE
suffix VARCHAR(10);
    SET
suffix = LPAD(FLOOR(RAND() * 10000000000), 10, '0');
...
prefix VARCHAR(255);
    SET
prefix = CONCAT(SUBSTRING('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789', FLOOR(1 + RAND() * 62), 1),
...
ip_address VARCHAR(20);
    SET
ip_address = CONCAT(FLOOR(RAND() * 256), '.', FLOOR(RAND() * 256), '.', FLOOR(RAND() * 256), '.', FLOOR(RAND() * 256));
RETURN ip_address;
END

-- 创建存储过程用于大批量插入测试数据
CREATE
DEFINER=`root`@`%` PROCEDURE `INSERT_USER`(max_num INT)
BEGIN
	DECLARE
i INT DEFAULT 0;
	#
...
VALUES (RAND_NAME(), RAND_NUM(0, 1), RAND_MOBILE());
UNTIL
i = max_num
END REPEAT;
COMMIT;
